...
*   **Vintage (YoY) burn/wind/nuclear curves:** ✅ Active — self-activating. The dashboard renders each panel from day one of retained history and switches the prior-year overlay on automatically the moment the archive spans a second calendar year (no config change; first full overlays land early 2027). The burn-vs-temp scatter already groups per vintage by year. Burn history needs its first winter (Dec 2026) before the heating arm of the sensitivity fit becomes meaningful — it is season-gated (Oct–Mar) so off-season HDD stragglers cannot produce a sign-flipped slope.
*   **Peaker efficiency drift (peaker share of burn by vintage):** ✅ Active — self-activating. The peak/off-peak dispatch-ratio trend renders now (with 30d rolling mean and the 1.4× heavy-dispatch line); the prior-year vintage overlay activates automatically once two calendar years of `peaker_history.csv` exist.
*   **Regional burn-vs-DD grossup scatters (Gulf/NE/Midwest/SE/West):** ❌ Pending. The desk-style per-region scatter needs (a) region-weighted temperature series — the current 79-city weighting is national — and (b) >6 months of per-ISO gas generation, which the 365-day retention window is accumulating. Re-evaluate after Jan 2027.
*   **Coal-vs-DD sensitivity curve:** ❌ Optional. Coal's temperature response is already visible through the gas-vs-coal switch scatter and gas-%-of-thermal series; a dedicated coal=f(T) fit becomes statistically meaningful with a full year of `thermal_history.csv` coal_mw data.
