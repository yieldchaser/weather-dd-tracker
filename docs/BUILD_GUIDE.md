# Weather DD Tracker â€” Advanced Weather Layer: Complete Build Guide
...
def score_regime(ao: float, nao: float, pna: float, epo: float = None) -> dict:
    ...
    """
    # Weights calibrated to HDD sensitivity â€” AO and PNA matter most for CONUS temps
...
The real edge â€” find historical years where the teleconnection state matched current, then extract what happened to temperatures in weeks 2-4.
...
on:
  schedule:
    - cron: '0 6,18 * * *'  # Twice daily â€” CPC updates once daily
  workflow_dispatch:
...
- The NAO forecast (week 1-4) is different from the NAO observation. Both are useful â€” don't conflate.
...
- Wallace & Gutzler (1981) original PNA paper â€” worth reading to understand what you're actually measuring
...
# This will be ~500MB for 40 years â€” do this ONCE, store locally
c.retrieve(
    'reanalysis-era5-pressure-levels',
    {
        ...
        'area': [60, -140, 20, -60],  # N, W, S, E â€” CONUS bounding box
        'format': 'netcdf',
...
def train_regime_model(nc_path: str, n_regimes: int = 10, season: str = "DJF"):
    ...
    Separate models per season recommended â€” atmospheric dynamics differ.
    ...
    # K-means clustering
    # Use k-means++ initialization â€” critical for reproducible clusters
...
def compute_persistence_days(transition_matrix: np.ndarray) -> dict:
    ...
    High persistence regimes (>5 days) are the ones worth flagging â€” 
...
- Michelangeli et al. (1995) â€” the foundational paper on weather regime classification in energy contexts
...
def fetch_noaa_hdd(region: str = "US", start_year: int = 2020) -> pd.DataFrame:
    ...
    # ... (format parsing â€” examine file first, format is stable but manual)
    pass
```
...
def compute_sensitivity_percentile(
    ...
    return {
        ...
        "interpretation": (
            "Weather surprises amplified â€” each HDD miss is worth more" if percentile > 70 else
            "Weather surprises muted â€” structural factors overriding temperature" if percentile < 30 else
...
The seasonal variation in Bcf/HDD slope (0.197 in Dec vs 0.249 in Oct) confirms the coefficient is not static. Your rolling OLS will capture this naturally. Target RÂ² > 0.70 for the regression to be reliable â€” below that, temperature has lost explanatory power (usually in shoulder seasons).
...
def build_capacity_factor_climatology(
    ...
    These grow annually â€” update yearly or use rolling peak as proxy.
    """
...
def compute_wind_drought_metrics(
    ...
    P25 is more robust than mean â€” less sensitive to extreme high events.
    """
...
def estimate_drought_persistence_probability(
    ...
    with anomalous ridging â€” the same regimes System 2 classifies.
    """
...
Images 4 and 7 (the Skylar wind CF seasonal and year-over-year charts) demonstrate exactly what you're computing. 2023's elevated winter wind CF wasn't noise â€” it was a persistent high-CF regime. Your streak counter distinguishes between a 2-day dip (noise) and a 7-day suppression event (signal). Image 6 (intraday swing) is worth tracking separately as a gas dispatch volatility proxy â€” the morning-to-evening MW swing indicates intraday gas commitment uncertainty.
...
```python
# CFS v2 Real-Time Forecasts on NOMADS â€” runs daily, 9-month horizon
...
def fetch_cfs_weekly_temp_outlook(lat: float, lon: float) -> pd.DataFrame:
    ...
    """
    # CFS runs daily â€” use most recent 00Z run
...
def compute_skill_weights(
    ...
    This is the standard approach â€” see Krishnamurti et al. (1999)
    ...
    try:
        ...
        # Insufficient data â€” return equal weights
...
|-------|---------|-----------------|-------------|
| Phase 1 (Weeks 1-2) | Freeze-off Trigger (3) + Dynamic Sensitivity (4) | Low â€” builds on existing EIA + GFS access | Immediate trading signal improvement |
| Phase 2 (Weeks 3-5) | Teleconnection Dashboard (1) + Wind Drought Persistence (5) | Low-Medium â€” all free APIs, simple math | Pattern lead-time extension |
| Phase 3 (Weeks 6-10) | Regime Classification (2) | Medium-High â€” requires ERA5 download + ML training | Highest information ceiling |
| Phase 4 (Ongoing) | Model Verification (7) | Low per run, cumulative value â€” starts paying off at ~90 days | Progressive ensemble improvement |
| Phase 5 (Month 3+) | Sub-Seasonal Bridge (6) | Medium â€” S2S data access learning curve | Week 3-6 edge, currently underserved |
...
```bash
pip install herbie-data          # Best GRIB2 download manager â€” use this
...
pip install sqlite3              # Built-in Python â€” forecast archive
```
