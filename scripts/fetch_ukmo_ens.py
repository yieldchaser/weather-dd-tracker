...
Fetches UK Met Office MOGREPS-G (18-member global ensemble) 2m temperature
using the Open-Meteo Ensemble API (`ukmo_global_ensemble_20km`).

Independent non-US, non-ECMWF ensemble source; diversifies the physics
consensus alongside GEFS / ECMWF ENS / CMC ENS.
...
def fetch_run(date_str, cycle):
    ...
    if out_path.exists():
        try:
            ...
            if len(existing) >= 6:
                print(f"  [SKIP] UKMO_ENS run {run_id}Z already fetched ({len(existing)} days).")
                return True
            else:
                print(f"  [WARN] UKMO_ENS {run_id}Z: only {len(existing)} rows — removing partial file.")
                out_path.unlink()
        except Exception:
            out_path.unlink()

    print(f"Syncing UKMO_ENS: {run_id} "
          f"(Open-Meteo ukmo_global_ensemble_20km, {len(DEMAND_CITIES)}-city batch)")
    ...
    if rows:
        if len(rows) < 6:
            print(f"  [WARN] UKMO_ENS {run_id}: only {len(rows)} day(s) — file NOT written.")
            return False
        ...
        print(f"  [OK] {run_id} UKMO_ENS: {len(rows)} days, "
...
def sync_all_ukmo():
    print("\n--- UK MET OFFICE ENSEMBLE SYNC SERVICE ---")
    now = datetime.datetime.now(datetime.UTC)

    # Determine the single active cycle based on current UTC time.
    # UKMO open-data lands on Open-Meteo ~4-6h after init.
    if now.hour >= 18:
