...
if not weights_file.exists():
    ...
    if result.returncode != 0:
        print("  ⚠  Gas-weight build failed — pipeline will use simple CONUS mean as fallback")
else:
...
if ecmwf_result.returncode != 0 and gfs_result.returncode != 0:
    print("\n⚠  Both ECMWF and GFS failed. Triggering Open-Meteo fallback...")
    fallback = subprocess.run(f"{PY} scripts/fetch_open_meteo.py", shell=True)
    if fallback.returncode != 0:
        print("✖ Open-Meteo fallback also failed. Exiting.")
        sys.exit(1)
    else:
