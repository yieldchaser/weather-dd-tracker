...
Fetches NOAA AIGEFS (Artificial Intelligence Global Ensemble Forecast System)
2m temperature using the Open-Meteo Ensemble API (`ncep_aigefs025`).

AIGEFS is a 31-member GraphCast-based AI ensemble, operational since Dec 2025.
It is distinct from AIGFS (deterministic) and HGEFS (62-member hybrid).
...
def fetch_run(date_str, cycle):
    ...
    if out_path.exists():
        try:
            ...
            if len(existing) >= 8:
                print(f"  [SKIP] AIGEFS run {run_id}Z already fetched ({len(existing)} days).")
                return True
            else:
                print(f"  [WARN] AIGEFS {run_id}Z: only {len(existing)} rows — removing partial file.")
                out_path.unlink()
        except Exception:
            out_path.unlink()

    print(f"Syncing AIGEFS: {run_id} "
          f"(Open-Meteo ncep_aigefs025, {len(DEMAND_CITIES)}-city batch)")
    ...
    if rows:
        if len(rows) < 8:
            print(f"  [WARN] AIGEFS {run_id}: only {len(rows)} day(s) — file NOT written.")
            return False
        ...
        print(f"  [OK] {run_id} AIGEFS: {len(rows)} days, "
...
def sync_all_aigefs():
    print("\n--- NOAA AIGEFS SYNC SERVICE ---")
    now = datetime.datetime.now(datetime.UTC)

    # Determine the single active cycle based on current UTC time.
    # NOAA AI models land on Open-Meteo ~3-4 hours after each init.
    if now.hour >= 22:
