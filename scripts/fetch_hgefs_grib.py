...
Purpose:
- Identify the latest available HGEFS run.
- Download ONLY the 2m temperature ensemble mean field using NOMADS .idx byte-range extraction.
...
def find_latest_available_runs(max_runs=3):
    ...
    for day_offset in [0, -1, -2, -3]:
        ...
        for cycle in CYCLES:
            ...
            test_file = f"hgefs.t{cycle}z.sfc.avg.f{last_fh_str}.grib2"
            test_url = f"{BASE_URL}/hgefs.{date}/{cycle}/ensstat/products/atmos/grib2/{test_file}"
            print(f"Checking HGEFS availability: {date}_{cycle}Z (f{last_fh_str})")
            if url_exists(test_url):
                print(f"[OK] Found available full HGEFS run: {date}_{cycle}Z")
    ...
    if not runs:
        ...
        if cached:
            print(f"[WARN] No remote HGEFS run found. Falling back to cached: {cached[0].name}")
            return []
        raise RuntimeError("No fully available HGEFS run found remotely or in local cache.")
    return runs
...
def fetch_latest_hgefs():
    print("\n--- HGEFS GRIB SYNC SERVICE ---")
    ...
    for run_date, cycle in available_runs:
        ...
        ensure_dir(run_dir)
        print(f"\nFetching HGEFS run: {run_id}Z (t2m only via byte-range)")
        ...
        for fh in FORECAST_HOURS:
            fh_str = f"{fh:03d}"
            base_name = f"hgefs.t{cycle}z.sfc.avg.f{fh_str}.grib2"
            base_url = f"{BASE_URL}/hgefs.{run_date}/{cycle}/ensstat/products/atmos/grib2/{base_name}"
        ...
        print(f"\nFetch complete for HGEFS {run_id}.")
